...
"""
Run PR-Light + seeded reasoning brain benchmark scenarios for Persistent Reasoning Light.

This script executes all configured scenarios in pr_light_brain mode using the
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run PR-Light Brain benchmark scenarios with a selectable backend."
    )
    parser.add_argument(
        "--backend",
...
def run_single(
    ...
    *,
    ...
    """
    Run one scenario in pr_light_brain mode and derive result.json from events.jsonl.
    """
    print(f"[RUN][pr_light_brain] scenario={scenario_id} backend={backend.backend}")
    ...
    json_path = result_json_path(paths, PR_LIGHT_BRAIN, scenario_id)
    ...
    print(f"[OK][pr_light_brain] result saved: {json_path}")


def run_all(repo_root: Path, *, backend: BackendSelection) -> None:
    """
    Run all configured pr_light_brain scenarios.
    """
...
def main() -> None:
    ...
    print("=== Run PR-Light Brain Benchmark ===")
    ...
    print()
    print("=== PR-Light Brain benchmark run complete ===")
