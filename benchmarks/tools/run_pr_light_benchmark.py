...
"""
Run PR-Light benchmark scenarios for Persistent Reasoning Light.

This script executes all configured scenarios in pr_light mode using the
...
from benchmark_adapters.common.benchmark_modes import PR_LIGHT
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run PR-Light benchmark scenarios with a selectable backend."
    )
    parser.add_argument(
        "--backend",
...
def run_single(
    ...
    *,
    ...
    """
    Run one scenario in pr_light mode and derive result.json from events.jsonl.
    """
    print(f"[RUN][pr_light] scenario={scenario_id} backend={backend.backend}")
    ...
    json_path = result_json_path(paths, PR_LIGHT, scenario_id)
    ...
    print(f"[OK][pr_light] result saved: {json_path}")


def run_all(repo_root: Path, *, backend: BackendSelection) -> None:
    """
    Run all configured pr_light scenarios.
    """
...
def main() -> None:
    ...
    print()
    print("=== PR-Light benchmark run complete ===")
